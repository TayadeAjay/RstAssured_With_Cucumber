...
public class UpdateBookingStepdefinition {
	...
	@Given("user creates a auth token with credential {string} & {string}")
	...
	@And("user updates the booking details using data {string} from Excel")
	...
	@And("user updates the booking details using data {string} from JSON file {string}")
	...
	@Then("user makes a request to update first name {string} & Last name {string}")
